...
public class HidDeviceApp
    ...
    boolean registerApp(BluetoothProfile inputHost, int mode) {
        ...
        if (mode == Constants.MODE_KEYBOARD) {
    ...
    /**
     * Send one raw keyboard report.
     *
     * <p>{@code BluetoothHidDevice.sendReport()} is not queued: when the L2CAP interrupt
     * channel is congested the stack discards the report and returns false. The result is
     * propagated so the caller can retry instead of losing a keystroke - or, far worse, a
     * key release.
     *
     * @return true if the stack accepted the report for transmission.
     */
    @Override
    @WorkerThread
    public boolean sendScancode(byte[] scancode) {
        ...
        BluetoothHidDevice host = inputHost;
        BluetoothDevice target = device;
        if (host == null || target == null) {
            Log.w(TAG, "sendScancode: no registered host or device, report not sent");
            return false;
        }

        boolean accepted = host.sendReport(target, Constants.ID_KEYBOARD, report);
        if (!accepted) {
            // Never log the report itself - it is the password in scancode form.
            Log.w(TAG, "sendScancode: stack refused report (channel congested?)");
        }
        return accepted;
    }
