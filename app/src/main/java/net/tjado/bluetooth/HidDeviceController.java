...
public class HidDeviceController
    ...
    /** Serialises Auto-Type transmissions without blocking the profile callbacks. */
    private final Object transmitLock = new Object();

    @SuppressLint("NewApi")
    private final Set<ProfileListener> listeners = new HashSet<>();
    ...
    private int currentMode = Constants.MODE_KEYBOARD;
    ...
    /**
     * Type a whole Auto-Type sequence to the connected HID host.
     *
     * <p>Blocking: it paces the reports and only returns once the host has been driven back to
     * an all-keys-up state. Must therefore be called from a worker thread, never from the main
     * thread.
     *
     * <p>The connection lock is deliberately not held for the duration of the transmission -
     * it guards the connection state and is taken by the profile callbacks, which must stay
    ...
    /**
     * Bridges {@link HidReportTransmitter} to the Bluetooth stack. Log messages carry counters
     * and status only - never report content, which is the password in scancode form.
     */
    private final HidReportTransmitter.ReportSink reportSink =
            new HidReportTransmitter.ReportSink() {
                @Override
                public boolean sendReport(byte[] report) {
                    return sendScancode(report);
                }
                ...
                public void log(String message) {
                    if (Log.isLoggable(TAG, Log.DEBUG)) {
                        Log.d(TAG, "hid-tx: " + message);
                    }
                }
            };

    /**
     * Send one raw 8-byte report.
     *
     * @return true if the Bluetooth stack accepted it for transmission. False means the report
     *         never reached the host and the caller has to retry it.
     */
    public boolean sendScancode(byte[] scancode) {
        synchronized (lock) {
            if (connectedDevice == null) {
                return false;
            }
            return hidDeviceApp.sendScancode(scancode);
        }
    }
