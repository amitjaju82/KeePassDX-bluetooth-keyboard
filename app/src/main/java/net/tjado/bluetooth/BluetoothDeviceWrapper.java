...
public final class BluetoothDeviceWrapper {
    ...
    /**
     * Local replacement for the Authorizer-specific hex helper, byte-for-byte identical to
     * net.tjado.passwdsafe.lib.Utils.bytesToHexString.
     *
     * Deliberately uses Integer.toHexString rather than String.format("%02x"): this value is
     * used as a SharedPreferences key, and String.format is locale-sensitive for numeric
     * conversions, so a locale with non-ASCII digits could produce a different key for the
     * same device and orphan the stored pairing.
     */
